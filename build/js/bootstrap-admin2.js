;(function () {
    ...
    //调色板侧边栏
    eventDelegate(document.body, 'click', 'div[class^=sidebarcolor]', function (event) {
        event.preventDefault();
    ...
    //调色板头部
    eventDelegate(document.body, 'click', 'div[class^=headercolor]', function (event) {
        event.preventDefault();
        event.stopPropagation();
    ...
    //给每个a链接都绑定一个自己的id
    var navAll = document.querySelectorAll('.bsa-sidebar-body > ul a:not(.has-children):not([target])');
    navAll.forEach(function (aEl) {
        aEl.dataset.pageid = guid();
    });

    //给带有.active的添加到右边的tab
    var withActiveAEl = document.querySelectorAll('.bsa-sidebar-body > ul a.active:not(.has-children):not([target])');

    withActiveAEl.forEach((el)=>{
        //得到当前a链接的href
        var href = el.getAttribute('href');
        var title = el.innerText;
        var pid = el.dataset.pageid;

        console.log(href,title,pid)
    ...
    //a链接不带有target属性的才匹配
    eventDelegate(document.body, 'click', '.bsa-sidebar-body > ul a:not(.has-children):not([target])', function (event) {
        event.preventDefault();

        //得到当前a链接的href
        var href = this.getAttribute('href');
        var title = this.innerText;
        var pid = this.dataset.pageid;
